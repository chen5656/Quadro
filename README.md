...
```bash
git clone https://github.com/your-username/Quadro.git
cd Quadro
npm install
...
## Deployment — run your own instance

The app runs on Cloudflare Pages (static frontend) and Cloudflare Workers + D1 (API, leaderboards, guest sessions).

Cloning this repo does **not** touch anyone else's database: D1, R2, and Workers are scoped to your Cloudflare account, and secrets are never committed. `wrangler.jsonc` (real deploy config) is gitignored — forks start from `wrangler.jsonc.example`.
...
npx wrangler d1 create quadro
```

Paste the returned `database_id` into your `wrangler.jsonc`, then set your domain:

- `routes` / `zone_name` → your domain, or delete the whole `routes` block to serve the Worker from `workers.dev` instead.
- `vars.ALLOWED_ORIGIN` → your Pages URL (e.g. `https://your-domain.com`).
- `seo/site.mjs` (`ORIGIN`) and the domain links in `content/legal/` → your domain.

Optional: `npx wrangler r2 bucket create quadro-avatars` for avatar storage. If you skip it, remove the `AVATARS` binding or deploy will fail.

### 2. Database

Apply the base schema plus every migration, in order — first locally, then `--remote` once that works:

```bash
npx wrangler d1 execute quadro --local --file worker/schema.sql
for f in worker/migrations/*.sql; do npx wrangler d1 execute quadro --local --file "$f"; done
# repeat with --remote
```

Auth is anonymous guest sessions via better-auth (`worker/auth/`), stored in D1 — no OAuth providers to configure. The only secret is generated by you:
...
```bash
npm run build
npx wrangler pages deploy dist --project-name quadro --branch release
npx wrangler deploy
```

### 4. Local dev with an API

```bash
npm run dev          # frontend on http://localhost:5173
npx wrangler dev --port 8787   # Worker + local D1; Vite proxies /api/* to it
```
...
Every sound effect and the menu bed are generated by `scripts/audio/build.mjs` and owned by this project. The two beds that play over a board are licensed:

- **Wonders of the Earth** by [Grand_Project](https://pixabay.com/music/adventure-wonders-of-the-earth-550792/) — Pixabay Content License (`public/audio/music/daily.mp3`).
- **Cinematic** by [Leberch](https://pixabay.com/music/orchestral-cinematic-582618/) — Pixabay Content License (`public/audio/music/practice.mp3`).

See `scripts/audio/build.mjs` for the full audio design.
