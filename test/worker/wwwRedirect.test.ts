 ...
 * Worker at all: Pages answered `/api/*` with the SPA fallback — `200` and
 * index.html — and the client threw parsing HTML as JSON.
 */
...
describe('the www redirect', () => {
  it('sends a www request back to the apex', async () => {
    const response = await call(new Request('https://www.acgame.win/api/guest'));
    expect(response.status).toBe(308);
    expect(response.headers.get('location')).toBe('https://acgame.win/api/guest');
  });
  ...
  it('redirects a POST without downgrading it to a GET', async () => {
    ...
    // turn a score submission into a silent no-op.
  ...
  it('leaves the apex alone', async () => {
    // `/api/providers` is one of the removed sign-in endpoints; the answer is
    // now the standard 404 rather than a provider list.
